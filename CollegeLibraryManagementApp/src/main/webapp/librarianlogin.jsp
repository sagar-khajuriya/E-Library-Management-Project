	...
	<div
		...
		<div class="libreg">
			...
			<center>
				...
				<br />
				<form method="post" action="./librarianlogincontroller">
					<table cellpadding="10px">
						<tr>
							<th>Librarian Id</th>
							<td><input type="text" name="lid"  <c:choose><c:when test="${loginLibrarian eq 'fail' }"> value="${lid }"</c:when></c:choose> /></td>
						</tr>
						<!--  <c:choose><c:when test="${loginLibrarian eq 'fail' }"> value="${lid }"</c:when></c:choose> -->
						<tr>
							<th>Password</th>
				...
				<c:choose>
					<c:when test="${loginLibrarian eq 'fail'}">
